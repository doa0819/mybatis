    ...
    public static void main(String[] args) {
        ...
        *   UnPooledDataSource : ConnectionPool 미사용*/
        ...
        Environment environment = new Environment(
                "dev"                     // 환경 정보 이름(id) -> 환경설정 key
                , new JdbcTransactionFactory()          // 트랜젝션 매니저 종류 결정(JDBC or MANAGED) - 커밋
                , new PooledDataSource(DRIVER, URL, USER, PASSWORD)              // ConnectionPool 사용 유무(Pooled or UnPooled) - 여러가지를 넣어 놓고 필요 한것을 쓴다
        );

        /* 필기. 생성한 환경 정보로 MyBatis 설정 객체 생성 */
        Configuration configuration = new Configuration(environment);

        configuration.addMapper(Mapper.class);          //mapper class 인터페이스 반환(인터페이스 직접 생성 불가)
        ...
        *   - false : Connection 인터페이스 타입 객체로 DML(insert, update, delete) 수행 후 auto commit  옵션 false(권장)*/
        ...
        Mapper mapper = sqlSession.getMapper(Mapper.class);        // 인터페이스 직접 생성 불가해서 get 이용 꺼내기

        mapper.selectSysdate();      // 리터럴 타입이여서 담아 줘야함 24-03-11
